...
const Panel = () => {
  ...
  useEffect(() => {
    void db.get('selected').then(e => {
      setSelectedSolution(e ?? []);
    });
  }, [modalShown]);
  ...
  const withdraw = async () => {
    ...
    const processQueue = async () => {
      ...
      for (let i = 0; i < maxConcurrentRequests && index < selectedSolution.length; i++) {
        await request(`/sadmin/api/article/promote/${selectedSolution[index++].id}?withdraw=1`, { method: 'POST' });
  ...
  return (
    <>
      ...
      {modalShown && (
        <Modal header='管理题解' long onSuccess={() => {
          setModalShown(false);
        }}
        >
